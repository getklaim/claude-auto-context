...
HYGIENE_DIR="$E2E_DIR/.claude-auto-context/hygiene"
...
rm -rf "$HYGIENE_DIR"
mkdir -p "$HYGIENE_DIR"
echo "  output dirs cleaned: rules/, offers/, hygiene/"
...
HYGIENE_BEFORE=$(find "$HYGIENE_DIR" -name '*.md' 2>/dev/null | sort)
...
HYGIENE_AFTER=$(find "$HYGIENE_DIR" -name '*.md' 2>/dev/null | sort)
...
NEW_HYGIENE=$(comm -13 <(echo "$HYGIENE_BEFORE") <(echo "$HYGIENE_AFTER") | grep '.' || true)

echo "  new rules files: $(echo "$NEW_RULES" | grep -c '.' || echo 0)"
echo "  new offer files: $(echo "$NEW_OFFERS" | grep -c '.' || echo 0)"
echo "  new hygiene files: $(echo "$NEW_HYGIENE" | grep -c '.' || echo 0)"
...
# L2: hygiene-agent — new .md file in hygiene/
if [ -n "$NEW_HYGIENE" ]; then
  HYGIENE_NAMES=$(echo "$NEW_HYGIENE" | xargs -I{} basename {} | tr '\n' ', ' | sed 's/,$//')
  pass "L2 hygiene-agent   (new: $HYGIENE_NAMES)"
else
  fail "L2 hygiene-agent   (no new files in .claude-auto-context/hygiene/)"
fi
